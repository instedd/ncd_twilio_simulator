require "spec"
require "../../src/models/ao_message"

describe Twiliosim::AOMessage do
  it "Say" do
    twiml = <<-XML
      <?xml charset="utf-8"?>
      <Response>
        <Say>command</Say>
      </Response>
    XML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)

    msg.crashed?.should eq(false)
    msg.get_digits?.should eq(nil)
    msg.play?.should eq(nil)
    msg.say?.should eq("command")
    msg.hangup?.should eq(false)
  end

  it "GetDigits + Say" do
    twiml = <<-XML
      <?xml charset="utf-8"?>
      <Response>
        <GetDigits timeout="10">
          <Say>command</Say>
        </GetDigits>
      </Response>
    XML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)

    msg.crashed?.should eq(false)
    msg.get_digits?.try(&.timeout).should eq(10)
    msg.play?.should eq(nil)
    msg.say?.should eq("command")
    msg.hangup?.should eq(false)
  end

  it "Say + Hangup" do
    twiml = <<-XML
      <?xml charset="utf-8"?>
      <Response>
        <Say>Thank you</Say>
        <Say language="en">.</Say>
      </Response>
    XML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)

    msg.crashed?.should eq(false)
    msg.play?.should eq(nil)
    msg.get_digits?.should eq(nil)
    msg.say?.should eq("Thank you")
    msg.hangup?.should eq(true)
  end

  it "Play (not supported)" do
    twiml = <<-XML
      <?xml charset="utf-8"?>
      <Response>
        <Play>http://host:port/path/to/audio.mp3</Play>
      </Response>
    XML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)

    msg.crashed?.should eq(false)
    msg.play?.should eq("http://host:port/path/to/audio.mp3")
  end

  it "crashed verboice" do
    twiml = <<-HTML
      <HTML>...</HTML>
    HTML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)

    msg.crashed?.should eq(true)
    msg.play?.should eq(nil)
    msg.get_digits?.should eq(nil)
    msg.say?.should eq(nil)
    msg.hangup?.should eq(false)
  end

  it "GetDigits num_digits=infinity" do
    twiml = <<-XML
    <?xml version="1.0"?>
    <Response>
      <GetDigits timeout="5" finishOnKey="" numDigits="infinity">
        <Say language="en">#numeric:1-120</Say>
      </GetDigits>
    </Response>
    XML

    msg = Twiliosim::AOMessage.new(twiml)
    msg.message.should eq(twiml)
    msg.received_at.should_not eq(nil)
    msg.get_digits?.try(&.timeout).should eq(5)
    msg.get_digits?.try(&.num_digits).should eq(Int32::MAX)
    msg.say?.should eq("#numeric:1-120")
  end
end
